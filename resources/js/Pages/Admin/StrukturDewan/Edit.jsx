...
export default function Edit({ strukturdewan }) {
    const { data, setData, post, processing, errors } = useForm({
        _method: "PUT",
        strukturdewan: "",
        deskripsi: strukturdewan.deskripsi || "",
    });

    const [previewThumbnail, setPreviewThumbnail] = useState(
        strukturdewan.thumbnail ? `/storage/${strukturdewan.thumbnail}` : null
    );
    const [previewstrukturdewan, setPreviewstrukturdewan] = useState(
        strukturdewan.strukturdewan ? `/storage/${strukturdewan.strukturdewan}` : null
    );

    const handleSubmit = (e) => {
        e.preventDefault();
        post(route("admin.strukturdewan.update", strukturdewan.id));
    };
    ...
    return (
        <AuthenticatedLayout>
            <Head title="Edit strukturdewan Organisasi" />
                ...
                <div className="flex items-center justify-between mb-10">
                    <div>
                        ...
                        <p className="text-gray-500 text-sm mt-1">
                            Perbarui informasi strukturdewan organisasi yang sudah ada.
                        </p>
                    </div>
                    <Link
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                    ...
                    <div className="bg-white rounded-2xl shadow-md border border-gray-100 p-6">
                        ...
                        <input
                            type="file"
                            accept="image/*"
                            onChange={(e) => handleImageChange(e, "strukturdewan", setPreviewstrukturdewan)}
                        ...
                        />
                        {errors.strukturdewan && (
                            <p className="text-red-500 text-sm mt-1">{errors.strukturdewan}</p>
                        )}
                        {previewstrukturdewan && (
                            <img
                                src={previewstrukturdewan}
                                alt="Preview strukturdewan"
                    ...
                    <div className="bg-white rounded-2xl shadow-md border border-gray-100 p-6">
                        ...
                        <textarea
                            ...
                            placeholder="Tulis deskripsi singkat tentang strukturdewan organisasi..."
                        ></textarea>
                        {errors.deskripsi && (
