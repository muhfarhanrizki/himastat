...
export default function Index({strukturdewan, dewans}) {

    const handleDeletePengurus = (id) => {
        if (confirm("Apakah Anda yakin ingin menghapus pengurus ini?")) {
            router.delete(route("admin.strukturdewan.destroy", id));
        }
    };

    return (
        <AuthenticatedLayout>
            <Head title="Dewan Musyawarah" />
            ...
            <div className="p-8 px-8 max-w-full mx-auto">
                ...
                <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-10">
                    ...
                    <div className="flex gap-3">
                        {strukturdewan ? (
                            <Link
                                href={route("admin.strukturdewan.edit", strukturdewan.id)}
                            ...
                            <Link
                                href={route("admin.strukturdewan.create")}
                                className="inline-flex items-center gap-2 bg-gray-700 hover:bg-gray-800 text-white px-5 py-2.5 rounded-lg shadow transition"
                            >
                ...
                {strukturdewan ? (
                    <div className="relative overflow-hidden rounded-2xl shadow-xl mb-8 h-96">
                        {strukturdewan.strukturdewan ? (
                            <img
                                src={`/storage/${strukturdewan.strukturdewan}`}
                                alt="strukturdewan Organisasi"
                    ...
                    <div className="relative overflow-hidden rounded-2xl shadow-xl mb-8 h-96  flex items-center justify-center">
                        <div className="text-center">
                            <Image size={80} className="mx-auto text-gray-400 mb-4" />
                            <p className="text-gray-600 text-lg mb-4">
                                Belum ada struktur dewan
                            </p>
                            <Link
                                href={route("admin.strukturdewan.create")}
                                className="inline-flex items-center gap-2 bg-gray-700 hover:bg-gray-800 text-white px-5 py-2.5 rounded-lg shadow transition"
                ...
                {strukturdewan && (
                    <div className="bg-white rounded-2xl shadow-md border border-gray-100 p-6 mb-8">
                        ...
                        <p className="text-gray-700 leading-relaxed whitespace-pre-line">
                            {strukturdewan.deskripsi || "Belum ada deskripsi yang ditambahkan."}
                        </p>
                    </div>
                )}
                ...
                <div>
                    <div className="flex items-center justify-between mb-6">
                        <h2 className="text-2xl font-bold text-gray-900">
                            Anggota Dewan ({dewans?.length || 0})
                        </h2>
                        <Link
                            href={route("admin.dewan.create")}
                            className="inline-flex items-center gap-2 bg-gray-700 hover:bg-gray-800 text-white px-5 py-2.5 rounded-lg shadow transition"
                        >
                            <PlusCircle size={18} /> Tambah Dewan
                        </Link>
                    </div>

                    {dewans && dewans.length > 0 ? (
                            ...
                            {dewans.map((person) => (
                                ...
                                >
                                    ...
                                    <div className="absolute top-3 right-3 z-10 flex gap-2 opacity-0 group-hover:opacity-100 transition">
                                        <Link
                                            href={route("admin.dewan.edit", person.id)}
